...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      jekyll: {
        files: [
          '<%= yeoman.app %>/**/*.{html,yml,md,mkd,markdown}', // '<%= yeoman.app %>/**/*.{html,yml,md,mkd,markdown}',
          // '_config.yml',
    ...
    buildcontrol: {
      dist: {
        options: {
          ...
          remote: 'git@github.com:veryrobert/theSuite.git',
          branch: 'gh-pages'
        }
      }
    },
      ...
      dist: {
        options: {
          patterns: [
            {
              match: /("|'?)\/?assets\//g,
              replacement: '$1http://veryrobert.github.io/theSuite/assets/'
            },
            {
              match: /(<a[^>]*href="?)(\/)/g,
              replacement: '$1http://veryrobert.github.io/theSuite/'
            },
            {
              match: /(<form[^>]*action="?)(\/)/g,
              replacement: '$1http://veryrobert.github.io/theSuite/'
